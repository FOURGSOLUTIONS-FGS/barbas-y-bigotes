...
export function NavInferior({
  ...
  return (
    <nav
      ...
      className={`fixed inset-x-3 bottom-[max(env(safe-area-inset-bottom),10px)] z-30 flex h-16 items-center gap-1 rounded-full border border-line bg-bg/85 px-1.5 backdrop-blur-md lg:hidden ${className}`}
    >
      {destinos.map((d) => {
        const act = d.clave === activo;
        const dentro = (
          <>
            <span aria-hidden className="relative [&>svg]:h-[22px] [&>svg]:w-[22px]">
              {d.icono}
            ...
            <span className="text-[12px] font-semibold leading-none">{d.etiqueta}</span>
          </>
        );
        const clases = `flex min-h-14 flex-1 flex-col items-center justify-center gap-1 rounded-full transition ${
