...
export function Kpi({
  ...
  return (
    <div className={`group relative overflow-hidden rounded-2xl border border-line bg-panel ${pad} transition hover:border-accent/30`}>
      <div className="pointer-events-none absolute -right-6 -top-6 h-20 w-20 rounded-full bg-ink/[0.04] blur-2xl" />
      ...
      <div className={`mt-2 font-display ${valueSize} tabular-nums ${accent ? "text-accent-soft" : "text-ink"}`}>{value}</div>
